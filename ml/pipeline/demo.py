...
class DemoScenarioGenerator:
    ...
    def next_frame(self, frame_index: int, anomaly_profile: str) -> tuple[list[Detection], list[TrackState], list[AnomalyEvent], list[dict]]:
        ...
        crowd_shift = 110 if frame_index > 55 else 0
        panic_multiplier = 2.5 if anomaly_profile == "panic" and frame_index > 70 else 1.0

        for idx, track_id in enumerate(self._track_ids):
            base_x = 120 + idx * 55
            x = base_x + math.sin(phase + idx) * 22 + crowd_shift
            y = 150 + (idx % 3) * 46 + math.cos(phase * 1.4 + idx) * 18
            ...
            heatmap_points.append(
                {
                    ...
                    "intensity": round(min(1.0, 0.45 + velocity / 20), 3),
                }
            )
        ...
        if frame_index == 8:
            events.append(
                AnomalyEvent(
                    ...
                    confidence=0.8,
                    description="Demo subject entered restricted zone near tourist gate",
