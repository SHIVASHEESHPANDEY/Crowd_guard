...
class CrowdAnomalyClassifier:
    ...
    def classify(self, detections: list[Detection], tracks: list[TrackState]) -> list[AnomalyEvent]:
    ...
    def _detect_movement_anomalies(self, tracks: list[TrackState]) -> list[AnomalyEvent]:
        ...
        for track in tracks:
            ...
            if track.velocity > max(45.0, average_velocity * 2.4) or abrupt_turn > 0.7:
        ...
        if len(high_velocity_people) >= 4:
            return [
                AnomalyEvent(
                    ...
                    confidence=0.71,
                    description="Collective panic-like behavior inferred from pose and motion surrogate features",
